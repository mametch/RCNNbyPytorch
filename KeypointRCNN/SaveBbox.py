...
parser.add_argument('--bbox_path', default='BboxOut', type=str,
                    help='input image path')
parser.add_argument('--key_path', default='KeyOut', type=str,
                    help='input image path')
...
for p in dir_list:
    ...
    os.makedirs(p.replace(args.dir_path, args.key_path), exist_ok=True)

img_list = [p for p in glob if os.path.isfile(p)]
for img_path in img_list:
    # File existence check
    name, ext = os.path.splitext(img_path)
    if os.path.isfile(name.replace(args.dir_path, args.key_path) + '.json'):
        continue
    ...
    # Save Keypoints
    with open(name.replace(args.dir_path, args.key_path) + '.json', 'w') as f:
        json.dump(KeyDict, f)
